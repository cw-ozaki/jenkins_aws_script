...
do
	...
	do
		...
		state_stopped_exists=`echo -n "${instance_json}" | jq -r 'select(.State == "stopped")' | wc -l`
		if [ ${tag_name_exists} -eq 0 -o ${exclude_tag_name_exists} -ne 0 -o ${state_stopped_exists} -eq 0 ];then
			continue
		fi
	...
	echo "aws ec2 start-instances --instance-ids ${instance_ids}"
	### dry-run
	if [ "${FLG_D}" == 'TRUE' ];then
		continue
	fi
